...
class EcephysSession(LazyPropertyMixin):
    ...
    @property
    def probewise_unit_counts(self):
        return self.units["probe_description"].value_counts()

    @property
    def structure_probe_wise_unit_counts(self):
        df = self.units[['ecephys_structure_acronym', 'probe_description']]
        return df.groupby(['ecephys_structure_acronym', 'probe_description']).size()

    @property
    def probe_structure_wise_unit_counts(self):
        df = self.units[['probe_description', 'ecephys_structure_acronym']]
        return df.groupby(['probe_description', 'ecephys_structure_acronym']).size()
    ...
    def map_probe_to_ecephys_structure_acronym(
            self,
            visual_only=False,
            verbose=False):
        """Builds a dict from probe to recorded areas."""
        probes = self.units['probe_description'].unique()
        map_probe_area = {}

        for p, probe in enumerate(probes):
            probe_units = self.units[self.units['probe_description'] == probe]
            all_areas = probe_units['ecephys_structure_acronym'].unique()
            if visual_only:
                map_probe_area[probe] = list(set(util.VISUAL_AREA) & set(all_areas))
            else:
                map_probe_area[probe] = all_areas
        if verbose:
            if visual_only:
                print('----------- Only visual ----------')
            else:
                print('----------- All recorded regions by probe ----------')
            for key in map_probe_area:
                print(f'{key}: {map_probe_area[key]}')
        return map_probe_area


    def select_units_by_mod_idx_dg(self, threshold):
        """Select neurons with large modulation index.
        ...
        pd.DataFrame :
        """
        return self.units[self.units['mod_idx_dg'] >= threshold]
    ...
    def unitwise_spike_histogram(
        ...
        tiled_data = tiled_data.mean(axis=0)  # Average across trials.
        if smooth_sigma is not None and smooth_sigma > 0:
            tiled_data = gaussian_filter1d(tiled_data, smooth_sigma, axis=0)  # Smooth along timeline.

        tiled_data = xr.DataArray(
            name='unitwise_histogram',
            data=tiled_data,
            coords={
                'time_relative_to_stimulus_onset': bin_edges[:-1],
                'unit_id': units.index.values
            },
            dims=['time_relative_to_stimulus_onset', 'unit_id']
        )
    ...
    def trialwise_spike_times(
        self, 
        stimulus_presentation_ids=None, 
        unit_ids=None,
        trial_time_window=None):
        """Get the spikes for the peri-stimulus.

        Args:
            stimulus_presentation_ids:
            unit_ids:
            trial_time_window: e.g [-0.5, 2.5]. The unit is second.
        ...
        presentation_start_times = np.array(stimulus_presentations['start_time'])
        presentation_end_times = np.array(stimulus_presentations['stop_time'])
        all_presentation_ids = np.array(stimulus_presentations.index.values)
        ...
        for unit_i, unit_id in enumerate(units.index.values):
            unit_spike_times = self.spike_times[unit_id]

            for s, stimulus_presentation_id in enumerate(stimulus_presentations.index.values):
                if trial_time_window is None:
                    trial_start_time = presentation_start_times[s]
                    trial_end_time = presentation_end_times[s]
                else:
                ...
                # e.g. np.searchsorted([1,2,3,4,5], 2.1) = 2
        ...
        onset_times = self._filter_owned_df(
            "stimulus_presentations", ids=all_presentation_ids)["start_time"]
        spikes_table = spike_df.join(
            onset_times, on=["stimulus_presentation_id"])
